 ...
 *
 * A polished, branded HTML email template for event RSVP reminders.
 * Uses inline CSS and table-based layout for maximum email client compatibility.
 * Follows the same pattern as invitation.tsx.
 */
...
/**
 * Generate the HTML email template for event reminders.
 * Uses inline CSS for maximum email client compatibility.
 */
export function renderReminderHtml(props: ReminderEmailProps): string {
  const {
    eventTitle,
    eventDate,
    eventTime,
  ...
  const greeting = safeGuestName ? `Hi ${safeGuestName},` : "Hi there,";
...
<head>
  ...
  <title>Reminder: RSVP for ${safeTitle}</title>
  <!--[if mso]>
  <noscript>
    <xml>
          ...
          <tr>
            <td style="padding: 32px 24px;">
              ...
              <p style="margin: 0 0 24px 0; color: #374151; font-size: 16px; line-height: 1.5;">
                Just a friendly reminder &mdash; <strong style="color: ${BRAND_COLOR};">${safeHostName}</strong> is waiting to hear from you about:
              </p>
              ...
              <p style="margin: 0 0 24px 0; color: #6b7280; font-size: 15px; line-height: 1.6;">
                The event is coming up soon! Please let us know if you can make it.
              </p>
          ...
          <tr>
            <td style="padding: 20px; text-align: center;">
              <p style="margin: 0; color: #9ca3af; font-size: 12px; line-height: 1.5;">
                You received this reminder because you were invited to an event on Shindig.
              </p>
            </td>
          </tr>
        </table>
...
/**
 * Generate a plain-text version of the reminder email.
 * Used as a fallback for email clients that don't support HTML.
 */
export function renderReminderText(props: ReminderEmailProps): string {
  const {
    eventTitle,
    eventDate,
    eventTime,
  ...
  // Reminder text
  lines.push(`Just a friendly reminder — ${hostName} is waiting to hear from you about:`);
  lines.push("");
  ...
  // Urgency text
  lines.push("The event is coming up soon! Please let us know if you can make it.");
  lines.push("");
...
/**
 * Generate the email subject line for reminders.
 */
export function getReminderSubject(eventTitle: string): string {
  return `Reminder: RSVP for ${eventTitle}`;
}

/**
 * Convenience function that returns both HTML and plain-text versions.
 */
export function renderReminder(props: ReminderEmailProps): {
  subject: string;
  html: string;
  text: string;
} {
  return {
    subject: getReminderSubject(props.eventTitle),
    html: renderReminderHtml(props),
    text: renderReminderText(props),
  };
}
